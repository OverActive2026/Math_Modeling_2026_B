...
function [dT,out] = thermal_temperature_state_ice( ...
    ...
    if isscalar(qaux)
        qaux = qaux*ones(g.N,1);
    end
    if numel(qaux) ~= g.N || any(~isfinite(qaux)) || any(qaux < 0)
        error('thermal_temperature_state_ice:InvalidAuxiliaryHeat', ...
            'qaux必须是非负有限标量或长度为温度网格数的向量。');
    end
    ...
    out.qgen = qgen;
    out.qaux = qaux;
    out.auxiliaryHeatFluxEquivalentWm2 = sum(qaux.*g.dx);
    out.qPhase = qPhase;
    out.qCond = qCond;
