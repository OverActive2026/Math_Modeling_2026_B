...
% [Q2-5] 累积电荷作为额外状态，dq/dt=j，单位为C/cm^2；问题4仅作统计。
% [Q3-1] 每片电热丝面功率独立给定，范围为0~1 W/cm^2。
% [Q3-2] 面功率按积分守恒关系int(qaux dx)=qk转换为等效体积热源。
% [Q3-3] 纯预热模式j=0；协同模式固定采用题设60 s线性升载曲线。
% [Q4-0] 支持initialState或initialStateFile传入逐网格预冷温度场；
%        当前加热输入仍保留问题3恒功率形式，供后续动态控制扩展。
%-------------------------------------------------------------------------------

function [result,model] = pemfc_stack5_simulate_q4( ...
    ...
    simOpt = set_default(simOpt,'initialState',[]);
    simOpt = set_default(simOpt,'initialStateFile','');
    ...
    if simOpt.nCells ~= 5
        error('pemfc_stack5_simulate_q4:FiveCellsRequired', ...
            '问题4固定为5片串联单电池。');
    end
    ...
    elseif strcmp(startupMode,'coheat')
        ...
        error('pemfc_stack5_simulate_q4:InvalidStartupMode', ...
    ...
    if numel(heaterPowerDensityWcm2) ~= simOpt.nCells || ...
        ...
        error('pemfc_stack5_simulate_q4:InvalidHeaterPower', ...
            '五片电热丝功率密度必须均位于[0,%.6g] W/cm^2。', ...
            simOpt.heaterMaxPowerDensityWcm2);
    end
    if ~isscalar(heatingDurationS) || ~isfinite(heatingDurationS) || ...
            heatingDurationS < 0
        error('pemfc_stack5_simulate_q4:InvalidHeatingDuration', ...
            '辅助加热持续时间必须是非负有限标量。');
    end
    ...
    %% 2. 建立五片单电池状态，并接入可选的预冷温度场

    nCells = simOpt.nCells;
    % 先建立网格和状态编号，再按每片温度场分别初始化气体浓度。
    gridInitOpt = simOpt.init;
    gridInitOpt.T0 = 273.15;
    [g,s] = pemfc_setup_ice(simOpt.nCellLayer,gridInitOpt);
    [initialTemperatureFieldK,initialEndPlateTemperatureK, ...
        initialStateInfo] = q4_resolve_initial_temperature( ...
        simOpt,g,nCells);
    spatialWeight = g.dx/sum(g.dx);
    initialCellAverageTemperatureC = ...
        (initialTemperatureFieldK.'*spatialWeight)-273.15;
    initialTemperatureC = mean(initialCellAverageTemperatureC);

    x0CellMatrix = zeros(s.Nx,nCells);
    initCell = cell(nCells,1);
    for k = 1:nCells
        initOpt = simOpt.init;
        initOpt.T0 = initialTemperatureFieldK(:,k);
        [gCell,sCell,x0CellMatrix(:,k),initCell{k}] = ...
            pemfc_setup_ice(simOpt.nCellLayer,initOpt);
        if gCell.N ~= g.N || sCell.Nx ~= s.Nx
            error('pemfc_stack5_simulate_q4:InternalGridMismatch', ...
                '逐片初始化时网格或状态维数发生变化。');
        end
    end
    ...
    if strcmpi(char(simOpt.endPlateModel),'separate')
        ...
        x0 = [x0CellMatrix(:);0;initialEndPlateTemperatureK(:)];
    else
        endPlateIndex = [];
        x0 = [x0CellMatrix(:);0];
    end
    ...
    constraints = struct('jMaxAcm2',simOpt.jMaxAcm2, ...
    ...
    rhs = @(t,x) stack_rhs_q4(t,x,strategy,auxiliaryHeating, ...
        ice,hydration,activation,transport,stackThermal,simOpt,g,s, ...
        cellStateIndex,chargeIndex,endPlateIndex);
    eventFunction = @(t,x) stack_events_q4(t,x,rhs,constraints);
    ...
    if simOpt.verbose
        fprintf(['\n开始计算问题4五片电堆：初始片均温范围=', ...
            '[%.3f, %.3f] degC，模式=%s\n'], ...
            min(initialCellAverageTemperatureC), ...
            max(initialCellAverageTemperatureC),startupMode);
    ...
    success = ~solverTerminatedUnexpectedly && successEvent && ...
        ...
        iceValid && poreOccupancyValid && currentValid && ...
    ...
    if success
        ...
        elseif any(eventIndex == 4) || ~currentValid
            stopReason = '电流密度达到或超过上限';
        elseif any(eventIndex == 5) || ~poreOccupancyValid
    ...
    % initialTemperatureC保留为五片初始平均温度，兼容原问题3汇总代码。
    result.initialTemperatureC = initialTemperatureC;
    result.initialTemperatureFieldK = initialTemperatureFieldK;
    result.initialTemperatureFieldC = initialTemperatureFieldK-273.15;
    result.initialCellAverageTemperatureC = ...
        initialCellAverageTemperatureC;
    result.initialEndPlateTemperatureK = initialEndPlateTemperatureK;
    result.initialEndPlateTemperatureC = ...
        initialEndPlateTemperatureK-273.15;
    result.initialStateInfo = initialStateInfo;
    ...
    model.g = g;
    model.s = s;
    model.init = initCell{1};
    model.initCell = initCell;
    model.x0Cell = x0CellMatrix(:,1);
    model.x0CellMatrix = x0CellMatrix;
    model.initialTemperatureFieldK = initialTemperatureFieldK;
    model.initialEndPlateTemperatureK = initialEndPlateTemperatureK;
    model.initialStateInfo = initialStateInfo;
    model.x0 = x0;
    ...
    model.rhs = rhs;
    model.rhsForPower = @(t,x,powerDensityWcm2) stack_rhs_q4( ...
        t,x,strategy,struct('powerDensityWcm2',powerDensityWcm2, ...
        'durationS',realmax, ...
        'maximumPowerDensityWcm2',simOpt.heaterMaxPowerDensityWcm2), ...
        ice,hydration,activation,transport,stackThermal,simOpt,g,s, ...
        cellStateIndex,chargeIndex,endPlateIndex);
    model.jacobianPattern = jacobianPattern;
    model.nonNegativeStates = nonNegativeStates;
...
function [dx,out] = stack_rhs_q4(t,x,strategy,auxiliaryHeating,ice, ...
    ...
    if ~isscalar(j) || ~isfinite(j) || j < 0
        error('pemfc_stack5_simulate_q4:InvalidCurrent', ...
            '加载策略必须返回非负有限标量电流。');
    end
    ...
    for k = 1:nCells
        ...
        [dxCell,cellOut{k}] = cell_rhs_q4(t,x(cellStateIndex{k}),j, ...
...
function [dx,out] = cell_rhs_q4(t,x,j,Tamb,heatBoundary,qaux,ice, ...
...
function [value,isTerminal,direction] = stack_events_q4( ...
    t,x,rhs,constraints)
    ...
    value = [minimumTemperatureC;voltageMargin;iceMargin; ...
...
function validate_stack_options(opt)
    ...
    positiveScalarFields = {'nCells','tMax','outputStep','RelTol','AbsTol', ...
        'MaxStep','jMaxAcm2','minimumVoltageV', ...
...
function plot_stack_result(result)
    ...
    figure('Name','Question 4: five-cell auxiliary cold start', ...
        'Color','w');
