...
function [result,model] = pemfc_stack5_simulate_q3( ...
    ...
    simOpt = set_default(simOpt,'enforceChargeCap',true);
    simOpt = set_default(simOpt,'iceScope','terminal');
    simOpt = set_default(simOpt,'temperatureEventMarginK',0.01);
    simOpt = set_default(simOpt,'stopOnSuccess',true);
    if ~ismember(char(simOpt.iceScope),{'terminal','path'})
        error('pemfc_stack5_simulate_q3:IceScope', ...
            'iceScope must be terminal or path.');
    end
    if ~isscalar(simOpt.temperatureEventMarginK) || ...
            ~isfinite(simOpt.temperatureEventMarginK) || ...
            simOpt.temperatureEventMarginK < 0
        error('pemfc_stack5_simulate_q3:TemperatureMargin', ...
            'temperatureEventMarginK must be nonnegative and finite.');
    end
    ...
    if strcmp(startupMode,'preheat')
        if heatingDurationS == 0
            error('pemfc_stack5_simulate_q3:ZeroPreheatDuration', ...
                'Pure preheating requires a positive heating duration.');
        end
        % The task does not prescribe a current after pure preheating.
        % Evaluate only the heating interval and its terminal state.
        simOpt.tMax = heatingDurationS;
    end
    ...
    eventFunction = @(t,x) stack_events_q3(t,x,rhs,constraints, ...
        chargeIndex,startupMode,simOpt);
    ...
    elseif min(initialOut.Vcell) < constraints.minimumVoltageV-1e-10
        initialEventIndex = 3;
    elseif strcmpi(simOpt.iceScope,'path') && ...
            max(initialOut.maxIceVolumeFractionCell) >= ...
    ...
    if isempty(initialEventIndex)
        if heatingDurationS > 0 && heatingDurationS < simOpt.tMax
            % An output point alone does not stop ode15s at a discontinuity.
            % Integrate the left limit with the heater on, then restart with
            % the complete state and the original (heater-off) right limit.
            heatingBeforeSwitch = auxiliaryHeating;
            heatingBeforeSwitch.durationS = simOpt.tMax+1;
            rhsBeforeSwitch = @(t,x) stack_rhs_q3(t,x,strategy, ...
                heatingBeforeSwitch,ice,hydration,activation,transport, ...
                stackThermal,simOpt,g,s,cellStateIndex,chargeIndex,endPlateIndex);
            eventsBeforeSwitch = @(t,x) stack_events_q3(t,x,rhsBeforeSwitch, ...
                constraints,chargeIndex,startupMode,simOpt);
            beforeOpt = odeset(odeOpt,'Events',eventsBeforeSwitch);
            [tSol,xSol,tEvent,xEvent,eventIndex] = ode15s(rhsBeforeSwitch, ...
                tOutput(tOutput <= heatingDurationS),x0,beforeOpt);
            switchTolerance = 100*eps(max(1,abs(heatingDurationS)));
            reachedSwitch = abs(tSol(end)-heatingDurationS) <= switchTolerance;
            [~,terminalFlags] = eventsBeforeSwitch(tSol(end),xSol(end,:).');
            terminalBeforeSwitch = any(terminalFlags(eventIndex));
            if reachedSwitch && ~terminalBeforeSwitch
                [tAfter,xAfter,tEventAfter,xEventAfter,eventIndexAfter] = ...
                    ode15s(rhs,tOutput(tOutput >= heatingDurationS), ...
                    xSol(end,:).',odeOpt);
                keepOutput = tAfter > tSol(end)+switchTolerance;
                tSol = [tSol;tAfter(keepOutput)];
                xSol = [xSol;xAfter(keepOutput,:)];
                keepEvent = true(size(tEventAfter));
                for ie = 1:numel(tEventAfter)
                    keepEvent(ie) = ~any(eventIndex == eventIndexAfter(ie) & ...
                        abs(tEvent-tEventAfter(ie)) <= switchTolerance);
                end
                tEvent = [tEvent;tEventAfter(keepEvent)];
                xEvent = [xEvent;xEventAfter(keepEvent,:)];
                eventIndex = [eventIndex;eventIndexAfter(keepEvent)];
            end
        else
    ...
    temperatureReached = all(TavgC(end,:) > ...
        simOpt.temperatureEventMarginK-1e-7);
    voltageValid = minimumCellVoltage >= ...
        constraints.minimumVoltageV-1e-8;
    iceValid = maxIceVolumeFractionCell(end,:) < ...
        constraints.maximumIceVolumeFraction;
    iceValid = all(iceValid) && ...
        (~strcmpi(simOpt.iceScope,'path') || ...
        maximumIceVolumeFraction < constraints.maximumIceVolumeFraction);
    poreOccupancyValid = maximumPoreOccupancy < ...
        constraints.maximumPoreOccupancy;
    currentValid = maximumCurrentDensity <= ...
        constraints.jMaxAcm2+1e-12;
    chargeValid = ~simOpt.enforceChargeCap || ...
        chargeCcm2(end) <= constraints.qMaxCcm2+1e-8;
    ...
    preheatDurationValid = true;
    if strcmp(startupMode,'preheat')
        atHeatingEnd = find(tSol >= heatingDurationS-1e-8,1,'first');
        preheatDurationValid = ~isempty(atHeatingEnd) && ...
            tSol(atHeatingEnd) <= heatingDurationS+1e-7 && ...
            all(TavgC(atHeatingEnd,:) > simOpt.temperatureEventMarginK) && ...
            all(maxIceVolumeFractionCell(atHeatingEnd,:) < ...
                constraints.maximumIceVolumeFraction) && ...
            min(min(Vcell(1:atHeatingEnd,:))) >= ...
                constraints.minimumVoltageV-1e-8;
        if preheatDurationValid
            temperatureReached = true;
            temperatureTargetTimeS = heatingDurationS;
            successEvent = true;
        end
    end
    ...
    terminalKinds = [3 5 6];
    if strcmp(startupMode,'coheat') && simOpt.stopOnSuccess
        terminalKinds(end+1) = 1;
    end
    if simOpt.enforceChargeCap, terminalKinds(end+1) = 2; end
    if strcmpi(simOpt.iceScope,'path'), terminalKinds(end+1) = 4; end
    terminalAtEnd = any(ismember(eventIndex,terminalKinds) & ...
        abs(tEvent-tSol(end)) < 1e-5);
    solverTerminatedUnexpectedly = ~requestedEndReached && ...
        ~terminalAtEnd;
    if strcmp(startupMode,'preheat')
        success = preheatDurationValid && ~solverTerminatedUnexpectedly && ...
            currentValid && chargeValid && poreOccupancyValid;
        questionSuccess = success;
    else
        failureBeforeSuccess = successEvent && any( ...
            ismember(eventIndex,terminalKinds( ...
            terminalKinds ~= 1)) & ...
            tEvent <= temperatureTargetTimeS+1e-7);
        questionSuccess = ~solverTerminatedUnexpectedly && successEvent && ...
            temperatureReached && voltageValid && iceValid && ...
            poreOccupancyValid && currentValid && chargeValid && ...
    ...
    result.questionSuccess = questionSuccess;
...
function [dx,out] = stack_rhs_q3(t,x,strategy,auxiliaryHeating,ice, ...
    ...
    if any(~isfinite(x))
        error('pemfc_stack5_simulate_q3:NonfiniteState', ...
            'Nonfinite ODE trial state at t=%.9g s.',t);
    end
    ...
    for k = 1:nCells
        ...
        try
            ...
            error('pemfc_stack5_simulate_q3:CellRhsFailure', ...
                'RHS failed at t=%.9g s, cell=%d, Tavg=%s K, endPlate=%s K: %s', ...
                t,k,mat2str(Tavg.',6), ...
                mat2str(endPlateTemperatureK.',6),ME.message);
        end
        if any(~isfinite(dxCell))
            error('pemfc_stack5_simulate_q3:NonfiniteDerivative', ...
                'Nonfinite derivative at t=%.9g s, cell=%d, j=%.6g A/m2, V=%.6g V, jLim=%.6g A/m2.', ...
                t,k,j,cellOut{k}.Vcell,cellOut{k}.thermal.jLim);
        end
...
function [value,isTerminal,direction] = stack_events_q3( ...
    t,x,rhs,constraints,chargeIndex,startupMode,simOpt)

    [~,out] = rhs(t,x);
    minimumTemperatureC = min(out.Tavg)-273.15- ...
        simOpt.temperatureEventMarginK;
    ...
    isTerminal = [~strcmp(startupMode,'preheat') && ...
        logical(simOpt.stopOnSuccess); ...
        logical(simOpt.enforceChargeCap);1; ...
        strcmpi(simOpt.iceScope,'path');1;1];
