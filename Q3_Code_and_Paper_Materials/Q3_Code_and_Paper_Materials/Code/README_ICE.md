...
> 注：本文件保留了复制时的第一题历史标定结果。
> `Code_Q2_PhaseFix`的当前机制、参数和验证结论以`README_PHASE_FIX.md`为准。
...
- `[INIT-2]`：将cCL初始离聚物含水量从1修正为2，并用-20/-25 degC两个零时刻电压联合重标定`j0Ref`和`Ea`。
...
```matlab
opt.init.lambdaCCL0 = 2.0; % 吹扫后cCL残留结合水含量
...
5. 将`lambdaCCL0`修正为2后，用-20/-25 degC实验在零时刻的0.799 V联合确定`j0Ref`与`Ea`，不沿用只适用于`lambdaCCL0=1`的旧活化参数。

重新标定后：

```matlab
j0Ref = 0.116300796747;   % [A/m^2]，298.15 K充分水合参考值
Ea = 17647.2467633;       % [J/mol]
fHydDry = 0.06;
lambdaHydOn = 3.5;
lambdaHydWet = 8.0;
nHyd = 3.0;
...
修正后的零时刻有效参考值为`0.116300796747*0.06=0.00697804780482 A/m^2`。这一调整同时修复了问题2高恒流下初始CL质子欧姆损失过大的问题。修改位置使用`[INIT-2]`标记。

加入有限排水边界并用`MaxStep=0.01 s`完整求解后，两工况结果为：

- -20 degC：零时刻电压`0.79900 V`，电压RMSE为`0.01623 V`，温度RMSE为`0.05584 degC`，全时段最大冰体积分数约`0.0465`。
- -25 degC：零时刻电压`0.79900 V`，电压RMSE为`0.01872 V`，温度RMSE为`0.15222 degC`，全时段最大冰体积分数约`0.1280`。
...
- 加入独立cCL水合状态后，`lambdaCCL0=2、tauHyd=10 s`时，初始CL质子电阻随产水水合自然衰减，不再使用人为时间开关。两工况全时段电压RMSE均低于0.019 V。
