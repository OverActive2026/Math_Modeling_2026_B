...
%            opt->可选初值：T0、lambda0、cH20、cO20、mw0、mi0
% @output:   g->空间网格
%            s->状态编号
%            x0->初始状态
%            init->初值汇总
...
function [g,s,x0,init] = pemfc_setup_ice(nCellLayer,opt)
    ...
    init.porousLayersPurged = all(mi0 == 0) && all(mw0(g.idx_porous) == 0);
