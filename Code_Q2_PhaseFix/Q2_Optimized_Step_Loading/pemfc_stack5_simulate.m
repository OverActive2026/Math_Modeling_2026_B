...
function status = stack_progress_output(t,~,flag,intervalS,tMax)
    ...
    % Show both simulation time and wall time. The OutputFcn is called
    % after accepted output steps, not during a stalled Newton iteration.
    persistent nextPrintTime startClock lastPrintWall
    status = 0;
    ...
    if strcmp(flag,'init')
        ...
        startClock = tic;
        lastPrintWall = 0;
        fprintf('仿真时间：%8.3f s（最长 %8.3f s），实际耗时：%6.1f s\n', ...
            t(1),tMax,0);
    elseif isempty(flag)
        currentTime = t(end);
        ...
        if isempty(startClock)
            startClock = tic;
            lastPrintWall = 0;
        end
        wallElapsed = toc(startClock);
        physicalTick = currentTime+10*eps(max(abs(currentTime),1)) >= ...
            nextPrintTime;
        wallTick = wallElapsed-lastPrintWall >= 10;
        if physicalTick || wallTick
            fprintf('仿真时间：%8.3f s（最长 %8.3f s），实际耗时：%6.1f s\n', ...
                currentTime,tMax,wallElapsed);
            lastPrintWall = wallElapsed;
        end
        if physicalTick
